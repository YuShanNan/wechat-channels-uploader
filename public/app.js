...
function connectWS() {
  ...
  socket.on('message', d => {
    ...
    if (d.type === 'account-updated') { loginLock = false; resetAddForm(); loadAccounts().then(updateAccountStatus); toast('登录成功', 'success'); }
  ...
  socket.on('connect', () => { if (!uploadRunning) updateAccountStatus(); });
  socket.on('disconnect', () => { setStatus('error', '连接断开'); setTimeout(connectWS, 2000); });
  socket.on('connect_error', () => { if (!uploadRunning) setStatus('error', '连接断开'); });
}
...
async function startUpload() {
  if (loginLock) return toast('请先完成扫码登录', 'warn');
  const account = $('accountSelect').dataset.value;
...
function renderAccounts() {
  ...
  accounts.forEach(a => {
    ...
    // Event handlers
    card.querySelector('[data-login]')?.addEventListener('click', () => { if (!loginLock) loginAccount(a.name); else toast('请先完成扫码登录', 'warn'); });
    card.querySelector('[data-verify]')?.addEventListener('click', () => { if (!loginLock) verifyAccount(a.name); else toast('请先完成扫码登录', 'warn'); });
    card.querySelector('[data-rename]')?.addEventListener('click', () => { if (!loginLock) editAccountLabel(a.name); else toast('请先完成扫码登录', 'warn'); });
    card.querySelector('[data-delete]')?.addEventListener('click', () => { if (!loginLock) deleteAccount(a.name); else toast('请先完成扫码登录', 'warn'); });
...
function renderAccountSelect() {
  ...
  const curName = sel.dataset.value || (accounts[0] && accounts[0].name) || '';

  // Update trigger button text
  const curAcct = accounts.find(a => a.name === curName);
  if (curAcct) {
    sel.dataset.value = curAcct.name;
    $('accountSelectText').textContent = curAcct.label + (curAcct.status === 'ready' ? '' : ' (未登录)');
  }

  // Populate dropdown
  const dd = $('accountSelectDropdown');
  dd.innerHTML = accounts.map(a =>
    '<button class="acct-select-option' + (a.name === curName ? ' selected' : '') + '" data-acct="' + esc(a.name) + '">' +
      '<span class="acct-select-dot ' + a.status + '"></span>' +
      esc(a.label) +
      (a.status === 'ready' ? '' : ' (未登录)') +
    '</button>'
  ).join('');
  ...
  dd.querySelectorAll('.acct-select-option').forEach(btn => {
    btn.addEventListener('click', () => {
      const name = btn.dataset.acct;
      sel.dataset.value = name;
      const acct = accounts.find(a => a.name === name);
      $('accountSelectText').textContent = (acct ? acct.label : name) + (acct && acct.status === 'ready' ? '' : ' (未登录)');
...
document.addEventListener('click', e => {
  if (!$('accountSelect').contains(e.target)) $('accountSelect').classList.remove('open');
});

function updateAccountStatus() {
  const name = $('accountSelect').dataset.value;
  const acct = accounts.find(a => a.name === name);
  if (!acct) return;
  if (acct.status === 'ready') setStatus('success', '已登录');
  else if (acct.status === 'needs-login') setStatus('idle', '未登录');
  else setStatus('idle', acct.status);
}
...
function resetAddForm() {
  ...
  $('addAccountBtn').textContent = '创建';
  $('cancelAddAccountBtn').textContent = '取消';
  $('cancelAddAccountBtn').disabled = false;
...
$('showAddAccountBtn').addEventListener('click', (e) => {
  e.stopPropagation();
  expandAddForm();
});
$('addAccountCard').addEventListener('click', (e) => {
  if (e.target.closest('button, input')) return;
  if ($('addAccountForm').style.display === 'none') expandAddForm();
});

$('cancelAddAccountBtn').addEventListener('click', (e) => {
  e.stopPropagation();
  loginLock = false;
  $('addAccountBtn').disabled = false;
  $('addAccountBtn').textContent = '创建';
...
$('addAccountBtn').addEventListener('click', async (e) => {
  ...
  if (!label) return toast('请输入账号名称', 'error');

  $('addAccountBtn').disabled = true;
  $('addAccountBtn').textContent = '创建中…';
  $('cancelAddAccountBtn').disabled = true;
  try {
    const res = await api('/api/accounts', {
      method: 'POST',
    ...
    // 锁定 UI，不加载账号列表（卡片等登录成功后再出现）
    loginLock = true;
    $('newAccountLabel').value = '';
    $('addAccountBtn').textContent = '等待扫码登录…';
    $('cancelAddAccountBtn').textContent = '取消登录';
    $('cancelAddAccountBtn').disabled = false;
    toast('正在启动登录窗口…', 'info');
    loginAccount(acct.name);
    toast('请在浏览器窗口中扫码', 'info');
  } catch (err) {
...
(function() {
  ...
  function applyTheme() {
    ...
    if (saved === 'dark')  { applyDark(); return; }
    if (saved === 'light') { applyLight(); return; }
    // 默认跟随系统
